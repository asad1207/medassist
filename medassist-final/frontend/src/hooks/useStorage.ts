...
export async function fetchStats(token: string) {
  try {
    const res = await fetch(`${API}/api/history/stats`, { headers: { Authorization: `Bearer ${token}` } });
...
export async function fetchAppointments(token: string): Promise<any[]> {
  try {
    const res = await fetch(`${API}/api/appointments/`, { headers: { Authorization: `Bearer ${token}` } });
    if (!res.ok) return [];
    return res.json();
  } catch { return []; }
}
...
export function getMeds(userId: string | number): MedEntry[] {
  try { const all = JSON.parse(localStorage.getItem(MED_KEY) || "{}"); return all[String(userId)] || []; } catch { return []; }
}
export function saveMed(userId: string | number, med: MedEntry) {
  try { const all = JSON.parse(localStorage.getItem(MED_KEY) || "{}"); all[String(userId)] = [med, ...(all[String(userId)] || [])]; localStorage.setItem(MED_KEY, JSON.stringify(all)); } catch {}
}
export function deleteMed(userId: string | number, id: string) {
  try { const all = JSON.parse(localStorage.getItem(MED_KEY) || "{}"); all[String(userId)] = (all[String(userId)] || []).filter((m: MedEntry) => m.id !== id); localStorage.setItem(MED_KEY, JSON.stringify(all)); } catch {}
}